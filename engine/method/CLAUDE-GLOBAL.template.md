<!--
  TEMPLATE — copy ONCE to ~/.claude/CLAUDE.md and fill in the <...>.
  Claude Code loads it automatically in ALL your projects and it's paid in
  tokens on EVERY message → only always-active rules go here.
  The close detail lives in /close (CLOSE-COMMAND.template.md): commands only
  load tokens when invoked.
  RECOMMENDED CAP: ~40 lines.
-->
<!-- method-version: 3.2 -->
...
## Brain
- Vault: `<vault-path>` — all method state lives there.
