...
**What v4.0 adds** (enforcement, gate, telemetry): task briefs carry
**acceptance criteria** and 0–2 skills from a curated registry
(`SKILLS-REGISTRY.md`); a SubagentStop hook (`check-brief.sh`) blocks
subagent reports without verification evidence; **`/gate`** runs risk-based
review lenses before any commit message (READY/NOT-READY, max 2 fix
cycles); real MR comments feed a per-project `review-checklist.md` via
`/learn review`; and every closed task leaves a metrics line — durations,
tokens by model, estimate vs. actual (`METRICS.md`).
...
| `~/.claude/hooks/metrics-event.sh` (SessionStart / SubagentStop / Stop) | Telemetry events (v4): timestamps for durations — never blocks | `scripts/metrics-event.sh` |
...
| `scripts/check-brief.sh` | `~/.claude/hooks/check-brief.sh` + registration in SubagentStop (once) |
| `scripts/metrics-event.sh` | `~/.claude/hooks/metrics-event.sh` + registration in SessionStart/SubagentStop/Stop (once) |
| `scripts/collect-metrics.sh` | not copied: `/close` runs it from here (per task) |
| `scripts/brain-health.sh` | not copied: run from here (weekly) |
| `scripts/brain-metrics.sh` | not copied: run from here (weekly — metrics rollup) |
...
| `MULTI-AGENT.md` | Parent → Subagents flow: roles, catalog, cycle, cycle cap, parallelism, fork and security |
| `SKILLS-REGISTRY.md` | Curated skill table for dispatches (v4): triggers, curation rules, no external fetching |
| `METRICS.md` | Telemetry (v4): what's captured, `metrics.jsonl` schema, how to read the rollup |
...
6. **Delegation with model routing.** Every delegable task is dispatched to a
   subagent with a task brief (`TASK-BRIEF.template.md`) — acceptance
   criteria + 0–2 registry skills — and with the cheapest model that solves
   it well (`MODEL-ROUTING.md`). The parent verifies the report before
   considering the task done (`MULTI-AGENT.md`).
7. **Gate before commit (v4).** Code destined for a commit/MR passes `/gate`
   on the integrated diff: risk-scaled review lenses + the project's
   `review-checklist.md`; only READY enables the commit message. Real MR
   comments feed the checklist via `/learn review`.
...
1. **Verify before marking `[x]`.** Run the appropriate build/test/lint and
   ...
   green, the task stays `[~]`. Non-trivial changes with a runtime surface
   are also exercised end-to-end (native `/verify` skill).
2. **Gate (v4).** Code destined for a commit/MR: `/gate` on the integrated
   diff must be READY.
3. Update the vault's `CONTEXT.md`: task `[x]`, new decisions, open questions,
   "Current state" with ISO date `YYYY-MM-DD`.
4. **Metrics (v4).** `collect-metrics.sh` + the human's `type`/`estimate` →
   a line in `metrics/metrics.jsonl` (`METRICS.md`). Never blocks the close.
5. **Record in the brain** what has cross-cutting value: non-obvious decision
   → `decisions/` (+ link in the hub); reusable learning → `patterns/` (see
   `BRAIN.md`).
6. Leave the commit message **ready to copy** (Conventional Commits, English,
   one line, ≤ 100 chars).

You don't move to the next task until the 6 points are done.
...
v4 adds two more harness-enforced pieces on the same principle:
**check-brief.sh** (SubagentStop) blocks a subagent's report that lacks
verification evidence (MULTI-AGENT.md), and **metrics-event.sh**
(SessionStart/SubagentStop/Stop) records telemetry timestamps — the latter
never blocks anything (METRICS.md).
